...
class Fp12 private[zksnark](var a: Fp6, var b: Fp6) extends Field[Fp12] {
  ...
  override def dbl: Fp12 = null

  private[zksnark] def mulBy024(ell0: Fp2, ellVW: Fp2, ellVV: Fp2) = {
    var z0 = a.a
    var z1 = a.b
    var z2 = a.c
    var z3 = b.a
...
//    var s1 = null
  ...
  override def add(o: Fp12) = new Fp12(a.add(o.a), b.add(o.b))

  override def mul(o: Fp12): Fp12 = {
    val a2 = o.a
    val b2 = o.b
    val a1 = a
    val b1 = b
  ...
  override def sub(o: Fp12) = new Fp12(a.sub(o.a), b.sub(o.b))

  override def inverse: Fp12 = {
    val t0 = a.squared
    val t1 = b.squared
  ...
  override def negate = new Fp12(a.negate, b.negate)

  override def isZero: Boolean = this == Fp12.ZERO
  ...
  private[zksnark] def cyclotomicSquared = {
    var z0 = a.a
    var z4 = a.b
    var z3 = a.c
    var z2 = b.a
...
//    var tmp = null
    ...
    var tmp = z0.mul(z1)
    var t0 = z0.add(z1).mul(z0.add(Fp6.NON_RESIDUE.mul(z1))).sub(tmp).sub(Fp6.NON_RESIDUE.mul(tmp))
    var t1 = tmp.add(tmp)
    // t2 + t3*y = (z2 + z3*y)^2 = b^2
    tmp = z2.mul(z3)
    var t2 = z2.add(z3).mul(z2.add(Fp6.NON_RESIDUE.mul(z3))).sub(tmp).sub(Fp6.NON_RESIDUE.mul(tmp))
    var t3 = tmp.add(tmp)
    // t4 + t5*y = (z4 + z5*y)^2 = c^2
    tmp = z4.mul(z5)
    var t4 = z4.add(z5).mul(z4.add(Fp6.NON_RESIDUE.mul(z5))).sub(tmp).sub(Fp6.NON_RESIDUE.mul(tmp))
    var t5 = tmp.add(tmp)
    // for A
    // z0 = 3 * t0 - 2 * z0
    z0 = t0.sub(z0)
  ...
  private[zksnark] def cyclotomicExp(pow: BigInteger) = {
    var res = Fp12._1
    ...
    while ( {
      ...
      if (pow.testBit(i)) res = res.mul(this)
  ...
  private[zksnark] def unitaryInverse = {
    val ra = a
    val rb = b.negate
    new Fp12(ra, rb)
  }

  private[zksnark] def negExp(exp: BigInteger) = this.cyclotomicExp(exp).unitaryInverse

  override def equals(o: Any): Boolean = {
    if (this == o) return true // loop?
    if (!o.isInstanceOf[Fp12]) return false
    val fp12 = o.asInstanceOf[Fp12]
    if (if (a != null) !(a == fp12.a)
    else fp12.a != null) return false
    //return !(b != null ? !b.equals(fp12.b) : fp12.b != null);
    if (if (b != null) !(b == fp12.b)
    else fp12.b != null) return false
    true
  }

  override def toString: String = String.format("Fp12 (%s; %s)\n" + "     (%s; %s)\n" + "     (%s; %s)\n" + "     (%s; %s)\n" + "     (%s; %s)\n" + "     (%s; %s)\n", a.a.a, a.a.b, a.b.a, a.b.b, a.c.a, a.c.b, b.a.a, b.a.b, b.b.a, b.b.b, b.c.a, b.c.b)
}
