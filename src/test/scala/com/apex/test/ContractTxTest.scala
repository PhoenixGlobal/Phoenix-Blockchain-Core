...
class ContractTxTest {  
  ...
  def testCreateChain(): Unit = {
    ...
    try {
      ...
      chain.startProduceBlock(producer.pubkeyHash, _miners.findPrivKey(producer).get, blockTime, Long.MaxValue)
      ...
      * */
      ...
      assert(chain.getBalance(producer.pubkeyHash).get == FixedNumber.fromDecimal(12.3))
      ...
      assert(chain.getBalance(producer.pubkeyHash).get == FixedNumber.fromDecimal(12.300021539))
