  ...
  def testChainFork5(): Unit = {
    ...
    try {
      ...
      * */
      ...
      val block3 = makeBlock(chain, block2A, Seq(makeTx(_acct1, _acct2, FixedNumber.fromDecimal(2), 2)))
      assert(tryInsertBlock(chain, block3))

      assert(chain.head.id() == block3.id())
